...
export interface WasmRuntimeHandlers {
...
async function initNodeWasmCodecs(): Promise<void> {
  ...
  const [jpegDecBytes, jpegEncBytes, pngBytes, webpDecBytes, resizeBytes] = await Promise.all([
    readFile(runtimeRequire.resolve('@jsquash/jpeg/codec/dec/mozjpeg_dec.wasm')),
    readFile(runtimeRequire.resolve('@jsquash/jpeg/codec/enc/mozjpeg_enc.wasm')),
    readFile(runtimeRequire.resolve('@jsquash/png/codec/pkg/squoosh_png_bg.wasm')),
    readFile(runtimeRequire.resolve('@jsquash/webp/codec/dec/webp_dec.wasm')),
    readFile(runtimeRequire.resolve('@jsquash/resize/lib/resize/pkg/squoosh_resize_bg.wasm')),
  ])
...
export const wasmRuntime: WasmRuntimeHandlers = {
  resolveInput: resolveWasmInput,
  decodeImage: decodeWasmImage,
  renderDataURL: renderWasmDataURL,
}

export function createWasmRuntimeHandlers(): WasmRuntimeHandlers {
  return wasmRuntime
}
